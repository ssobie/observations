...
make_obs_bc_albers_plot <- function(var.name,plot.type,seas,plot.title,plot.data,shared.range=NULL,
   ...
   if (var.name == 'tasmax' & plot.type == 'past' & seas == 'December') {
      class.breaks <- seq(-40,10,2)
   }
   if (var.name == 'tasmax' & plot.type == 'past' & seas == 'January') {
      class.breaks <- seq(-34,10,2)
   }
...
##plot.file <- paste0(plot.dir,'PRECIP.',seas,'.Total.Obs.Comparison.png')
plot.file <- paste0(plot.dir,'TASMAX.',seas,'.Average.Obs.Comparison.png')
##plot.file <- paste0(plot.dir,'TASMIN.',seas,'.Average.Obs.Comparison.png')
...
##
  ...
  rv <- make_obs_bc_albers_plot(var.name=var.name,plot.type='past',seas=seas,
  ...
  rv3 <- make_obs_bc_albers_plot(var.name=var.name,plot.type='past',seas=seas,
                          plot.title=paste0(seas,'\n',agg,'\n(ANUSPLIN)'),aplin.data,
                          yshort=TRUE)
  rv4 <- make_obs_bc_albers_plot(var.name=var.name,plot.type='past',seas=seas,
                          plot.title=paste0(seas,'\n',agg,'\n(Daymet)'),daymet.data)
                          
  rv5 <- make_obs_bc_albers_plot(var.name=var.name,plot.type='past',seas=seas,
